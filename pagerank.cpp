...
pagerank_v pagerank_algorithms::get_pers_pagerank(const double C=0.85, const double eps=1e-4, const int max_iter=100)
{
	// initialize
	const unsigned int nnodes = g.get_num_nodes();
	pagerank_v pagerankv(nnodes);
	...
	// pagerankv[nnodes].pagerank = 1;
	// pagerankv[nnodes + 1].pagerank = 0;

	// Compute Red personilized for each node.
	std::vector<double> tmp_pagerank(nnodes, 0);
	int iter = 0;
	for (; iter < max_iter; ++iter) {
		double sum = 0.0;
		for (node = 0; node < nnodes; ++node) {
			...
			for (const int &neighbor : g.get_in_neighbors(node)) {
				...
				if (neigh_degree > 0) {
					...
					tmp_pagerank[node] += pagerankv[neighbor].pagerank / nnodes;
				}
			}
			tmp_pagerank[node] *= C;
			tmp_pagerank[node] += pagerankv[node].pagerank;
		}
