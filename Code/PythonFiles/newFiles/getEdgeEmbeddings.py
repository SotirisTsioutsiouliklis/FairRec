...
Examples:
    the general syntax is the following:
        ...
        >>> python3 getEdgeEmbeddings.py -i <node embedding file> -e <edge file> -p <edge embedding policy> -o <output file> [-a <Sources column> <Targets column>]
...
if __name__ == "__main__":
    ...
    if (not len(sys.argv) != 9) and (not len(sys.argv) == 12):
        InputErrors.argumentError()

    # Check obligatory arguments.
    if sys.argv[1] == "-i" and sys.argv[3] == "-e" and sys.arv[5] == "-p" and sys.argv[7] == "-o":
        ...
        if len(sys.argv) == 12:
            if sys.argv[9] == "-a":
                edge_columns = [sys.argv[10], sys.argv[11]]
            else:
                InputErrors.argumentError()
        else:
    ...
    if policy == "hadamart":
        ...
        edgeEmbeddings = EdgeEmbeding.concatenate(edges)
    else:
