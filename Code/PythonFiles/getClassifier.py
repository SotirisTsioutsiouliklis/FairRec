...
Examples:
    ...
    >>> python3 getClassifier.py -p positive -n <negative edge sample file> -o <output file>
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "-p" and sys.argv[3] == "-n" and sys.argv[5] == "-o":
    ...
    # Create X, Y.
    positive_sample = (
        pd.read_csv(positive_edge_file)
        .drop(columns=["Sources", "Targets"], axis=1)
        .to_numpy()
    )
    negative_sample = (
        pd.read_csv(negative_edge_file)
        .drop(columns=["Sources", "Targets"], axis=1)
        .to_numpy()
    )
    ...
    Y = np.concatenate(
        (
            [1 for i in range(len(positive_sample))],
            [0 for i in range(len(negative_sample))],
        )
    )
