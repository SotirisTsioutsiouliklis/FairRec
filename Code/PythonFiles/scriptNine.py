    ...
    if sys.argv[1] == '-s':
        for directory in directories:
            for i in range(10):
                ...
                subprocess.call(["cp ../../Executables/PythonFiles/getEdgeRecommendationScore.py %s/network_%d" %(directory, i)], cwd=".", shell=True)
                subprocess.call(["python3 getEdgeRecommendationScore.py"], cwd= "%s/network_%d" %(directory, i), shell=True)
    elif sys.argv[1] == '-r':
        for dataset in datasets:
                # Copy Executables.
                print("In dataset: ", dataset)
                subprocess.call("cp ../Executables/PythonFiles/getEdgeRecommendationScore.py %s" %dataset, cwd=".", shell=True)
                # Execute it.
                subprocess.call("python3 getEdgeRecommendationScore.py", cwd="%s" %dataset, shell=True)
    else:
        sys.exit("Options:\n\tpython3 exp<Number>.py -s : For synthetic datasets\n\tpython3 exp<Number>.py -r : For real datasets")