""" Automatization script to get source nodes for the experiments.
"""
import subprocess
import sys
...
if len(sys.argv) != 2:
    ...
    if sys.argv[1] == '-s':
        ...
        for dataset in datasets:
            ...
            subprocess.call(["cp ../Executables/CppFiles/getSourceNodes.out %s" %dataset], cwd=".", shell=True)
            subprocess.call(["./getSourceNodes.out"], cwd= "%s" %dataset, shell=True)
    else:
        sys.exit("Options:\n\tpython3 exp<Number>.py -s : For synthetic datasets\n\tpython3 exp<Number>.py -r : For real datasets")